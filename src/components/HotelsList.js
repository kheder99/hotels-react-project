...
export default function HotelsList(props) {
    const addHotel = localStorage.getItem('role') ==="Admin"?<Link to="/addHotel"><button className="btn-primary " type="submit" >Add Hotel</button></Link>:null;
    const hotels = props.hotels.map(hotel => {
        if(hotel) {return <Hotel key={hotel.id} hotel={hotel}/>}
    })
    if(hotels.length === 0) {
